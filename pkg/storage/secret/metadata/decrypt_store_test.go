...
func TestIntegrationDecrypt(t *testing.T) {
	...
	t.Run("when auth info is not in allowlist, it returns an unauthorized error", func(t *testing.T) {
		...
		newSecureValue(authCtx, t, secureValueMetadataStorage, sv)
...
func newSecureValue(ctx context.Context, t *testing.T, db contracts.SecureValueMetadataStorage, sv *secretv0alpha1.SecureValue) {
	t.Helper()
